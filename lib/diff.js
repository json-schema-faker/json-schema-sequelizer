...
function buildSchema(reference, models, schema, source, prev, raw) {
  ...
  function addColumns(props, out) {
    Object.keys(props).forEach(prop => {
      ...
      if (ref.$ref || (ref.items && ref.items.$ref)) {
        out.push(`${pad}    // ${prop} <${ref.$ref || ref.items.$ref}>`);
        return;
      }
  ...
  walk(schema, source, prev, (key, type, value, previous, sourceObject) => {
      ...
      if (type === CREATED) {
        ...
        if (sourceObject.properties && sourceObject.properties.$type === 1) {
          ...
          if (options.paranoid !== false) {
            addTimestamp('deletedAt', tmp);
          }

          /* istanbul ignore else */
          if (options.timestamps !== false) {
            addTimestamp('createdAt', tmp);
            addTimestamp('updatedAt', tmp);
          }
        }
    ...
    if (key[0] === 'properties' && key[1] && type !== 0 && !ref) {
      ...
      if (type === CREATED) {
        if (!key[2]) {
          ...
          change.push(`${pad}() =>\n${pad}  queryInterface.changeColumn('${tableName}', '${key[1]}', {`);
    ...
    if (key[0] === 'options') {
      ...
      if (type === CREATED) {
        /* istanbul ignore else */
        if (key[1] === 'paranoid') {
          addTimestamp('deletedAt', up, down);
        }

        /* istanbul ignore else */
        if (key[1] === 'timestamps') {
          addTimestamp('createdAt', up, down);
          addTimestamp('updatedAt', up, down);
        }
      }
    }
  ...
  /* istanbul ignore else */
  if (raw) {
    return { up, down, change, reference };
  }
...
function doChanges(model, refs, from, to, m, p, s) {
  return buildSchema(model, refs, m, from, shadowClone(from, to), p, s);
}
