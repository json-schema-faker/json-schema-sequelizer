...
function buildSchema(schema, source, prev) {
  ...
  function addColumn(prefix, field, out) {
    ...
    /* istanbul ignore else */
    if (field.enum || (field._modelAttribute && field.type.toString() === 'ENUM')) {
      enumValues = field.type.values;
    }
  ...
  const tableName = schema._tableName.$data;
  ...
  walk(schema, source, prev, (key, type, value, previous, sourceObject) => {
    ...
    if (key[0] === 'id') {
      ...
      if (type === CREATED) {
        ...
        up.push(`${pad}}${opts}),`);
        down.push(`${pad}queryInterface.dropTable('${sourceObject._tableName.$data}'${opts}),`);
      }

      /* istanbul ignore else */
      if (type === DELETED) {
        // FIXME: adjust original name back...
        console.log(source, prev);
