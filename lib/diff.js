    ...
    Object.keys(props).forEach(prop => {
      ...
      if (ref.$ref || (ref.items && ref.items.$ref)) {
        ...
        if (_assoc.isSingleAssociation) {
          out.push(`${pad}  ${_assoc.foreignKey}: dataTypes.${
            _assoc.target.attributes[_assoc.targetKey].type
          }, // ${_assoc.associationType} <${_ref}>`);
        } else {
          out.push(`${pad}  // ${prop} ${_assoc.associationType} <${_ref}>`);
        }
        return;
      }
