...
function buildSchema(tableName, schema, source, prev) {
  const pad = '    ';
  ...
  function addColumn(prefix, field, out) {
    ...
    /* istanbul ignore else */
    if (field.enum || sub[0] === 'ENUM' || (field._modelAttribute && field.type.toString() === 'ENUM')) {
      enumValues = field.type.values || field.enum;
    }
  ...
  function addColumns(props, out) {
    Object.keys(props).forEach(prop => {
      ...
      if (ref.$ref || (ref.items && ref.items.$ref)) {
        out.push(`${pad}  // ${prop} <${ref.$ref || (ref.items && ref.items.$ref)}>`);
        return;
      }
  ...
  walk(schema, source, prev, (key, type, value, previous, sourceObject) => {
    ...
    if (key[0] === 'id') {
      /* istanbul ignore else */
      if (type === CREATED) {
        up.push(`${pad}queryInterface.createTable('${tableName}', {`);
        ...
        if (sourceObject.properties && sourceObject.properties.$type === 1) {
          ...
          /* istanbul ignore else */
          if (options.paranoid) {
            up.push(`${pad}  deletedAt: {`);
            up.push(`${pad}    type: dataTypes.DATE,`);
            up.push(`${pad}  },`);
            down.unshift(`${pad}queryInterface.removeColumn('${tableName}', 'deletedAt'),`);
          }
        }
...
function doChanges(table, from, to, m, p) {
  return buildSchema(table, m, from, shadowClone(from, to), p);
}
