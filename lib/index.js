...
// wrap methods
const METHODS = [
  'hooks', 'scopes', 'defaultScope',
  'references', 'classMethods', 'instanceMethods',
];
...
module.exports = function JSONSchemaSequelizer(sequelize, refs, cwd, $) {
  ...
  function makeModel($schema, model, conn) {
    ...
    METHODS.forEach(key => {
      model[key] = typeof model[key] !== 'undefined'
        ? util.call(model[key], $, conn)
        : undefined;
    });
  ...
  glob.sync('**/*.{js,json}', { cwd, nodir: true }).forEach(model => {
    ...
    // unwrap definition
    modelDefinition = model.indexOf('.json') > -1
      ? { $schema: modelDefinition }
      : util.call(modelDefinition, $);
  ...
  const fixedRefs = {
    ...
    canRead: true,
    read(file, callback) {
      callback(null, $refs[file.url] || $refs[path.relative(cwd, file.url)]);
    },
  };
  ...
  return Promise.all(_models.map(def =>
    $RefParser.dereference(cwd, def.schema, {
      ...
      _defns[_schema.id] = makeModel(_schema, def.model, sequelize);
    })))
  .then(() => {
    // apply relationships
