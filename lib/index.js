...
const PROPERTIES = ['sourceKey', 'targetKey', 'foreignKey', 'otherKey'];
...
function makeModel(sequelize, $schema, props, refs, rel, $) {
  ...
  F.model.fake = fakeSchema(_resolvedSchema, refs);
  // F.id = _modelName;
...
module.exports = (sequelize, defns, refs) => {
  ...
  Object.keys(rel).forEach((a) => {
    rel[a].forEach((b) => {
      ...
      PROPERTIES.concat('as').forEach((_prop) => {
        if (b[_prop]) {
          params[_prop] = b[_prop];
        }
      });

      METHODS.forEach((_method) => {
        if (b[_method]) {
          models[a].model[_method](models[b[_method]].model, params);
        }
      });
    });
  });
