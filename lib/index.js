  ...
  function hydrate(bundle) {
    ...
    Object.keys(bundle).forEach(ref => {
      ...
      if (bundle[ref].properties) {
        ...
        _defns[ref].$schema.references = {
          primaryKey: _models[ref].model.hasPrimaryKeys ? {
            prop: pk,
            type: fields[pk] ? fields[pk].type : null,
          } : null,
        };

        // track refs as dependencies
        _defns[ref].$schema.dependencies = _models[ref].refs;
      }
    });
