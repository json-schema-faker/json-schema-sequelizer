...
function JSONSchemaSequelizer(settings, refs, cwd) {
  ...
  const fixedRefs = {
    ...
    canRead: true,
    read: (file, callback) =>
      callback(null, (_defns[file.url] || _defns[path.relative(cwd, file.url)]).$schema),
  };
    ...
    this.connect()
      .then(() => {
        ...
        Object.keys(bundle).forEach(ref => {
          /* istanbul ignore else */
          if (bundle[ref].$ref === ref) {
            _models[ref] = util.makeModel(ref, conn, _defns[ref], _defns[ref].$schema);
          }
        ...
        // apply relationships
        Object.keys(_models).forEach(m => {
          util.makeRefs(_models[m], _models);
        });
      })
      .then(() => this);
  ...
  this.migrate = dump =>
    this.connect()
      .then(() => {
        ...
        return new $RefParser()
          .dereference(cwd, dump, fixedOpts)
