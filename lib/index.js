...
function JSONSchemaSequelizer(settings, refs, cwd) {
  ...
  Object.defineProperty(this, 'schemas', {
    ...
    enumerable: true,
    get: () => {
      return Object.keys(_defns).reduce((prev, cur) => {
        return prev.concat(util.copy(_defns[cur].$schema, util.mixRefs));
      }, []);
    },
  });
  ...
  this.connect = () => {
    ...
    const maxTries = settings.retries || 50;
      ...
      return this.sequelize
        ...
        .catch(err => {
          if (err.name === 'SequelizeConnectionError'
            && (err.message.includes('terminated unexpectedly') || err.message.includes('starting up'))) {
            return new Promise(next => setTimeout(() => {
              ...
              retries += 1;
              next();
            }, settings.timeout || 200)).then(() => waitForConn());
          }
      ...
      .then(() => {
        ...
        const _refs = Object.keys(_defns).reduce((prev, cur) => {
          ...
          if (_defns[cur].$schema) {
            prev.push(new $RefParser().dereference(cwd, util.copy(_defns[cur].$schema), fixedOpts));
          }
  ...
  // migration helper
  this.rehydrate = dump => this.connect()
    .then(() => {
