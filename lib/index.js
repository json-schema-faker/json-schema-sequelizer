...
const AUTOLOAD = {
  ...
  'mutators.js': 'graphqlMutators',
  'resolvers.js': 'graphqlResolvers',
};
...
function JSONSchemaSequelizer(settings, refs, cwd) {
  ...
  const fixedRefs = {
    ...
    read: (file, callback) => {
      ...
      if (!schema) {
        throw new Error(`Missing '${rel}' definition (${file.url})`);
      }
    ...
    glob.sync('**/*.js', { cwd, nodir: true }).forEach(model => {
      ...
      Object.keys(AUTOLOAD).forEach(src => {
        ...
        if (fs.existsSync(def)) {
          modelDefinition[AUTOLOAD[src]] = def.indexOf('.graphql') > -1
            ? fs.readFileSync(def).toString().trim()
            : require(def);
        }
      });
...
// graphql support
JSONSchemaSequelizer.graphql = require('./graphql');
