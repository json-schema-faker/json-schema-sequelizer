...
function JSONSchemaSequelizer(settings, refs, cwd) {
  ...
  function assocTypes(model, bundle) {
    const keys = Object.keys(model.associations);
    const props = model.options.$schema.properties;

    return keys.reduce((memo, cur) => {
      if (props[cur].items) {
        if (props[cur].items.id) {
        /* istanbul ignore else */
          if (!bundle[props[cur].items.id]) memo[cur] = { items: { $ref: props[cur].items.id } };
        } else {
          memo[cur] = { items: props[cur].items };
        }
      } else if (props[cur].id) {
        /* istanbul ignore else */
        if (!bundle[props[cur].id]) memo[cur] = { $ref: props[cur].id };
      } else {
        memo[cur] = props[cur];
      }
      return memo;
      ...
      if (bundle[ref].properties || bundle[ref].type) {
        ...
        _tasks.push(() => {
          ...
          _defns[ref].$references = {
            associations: assocTypes(_models[ref].model, bundle),
  ...
  this.connect = () => {
    ...
    settings.dialect = settings.dialect || settings.connection.split(':')[0];

    if (settings.logging) {
      settings.logging(`\rWaiting ${settings.dialect} for connection...\x1b[K`);
    }
  ...
  this.resource = (name, options) => {
    return JSONSchemaSequelizer.resource(this, options, name);
  };
}
