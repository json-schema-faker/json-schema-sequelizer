...
function _getOpts(model, action, params) {
  ...
  if (action === 'update' || action === 'create' || action === 'destroy') {
    ...
    return mix(params, {
      include: Object.keys(model.associations).map(ref => {
        const o = _getOpts(model.associations[ref].target, action);
        o.model = model.associations[ref].target;
        o.as = ref;
        return o;
      }),
    });
    ...
    .then(() => {
      ...
      switch (action) {
            ...
            .then(() => {
              ...
              return Promise.all(Object.keys(_props)
                .map(key =>
                  ...
                  && (payload[key].id
                    ? model.associations[key].target.update(payload[key], {
                      where: { id: payload[key].id },
                    })
                    : model.associations[key].target.create(util.merge(_pk, payload[key]))
                  )))
        ...
        case 'create':
          ...
          console.log(_fixParams(model.options.$schema, ctx.params.payload, _props), _getOpts(model, action));
