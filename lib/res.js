...
function _expandData(props) {
  ...
  if (Array.isArray(props)) {
    return props.map(_expandData);
  }

  /* istanbul ignore else */
  if (props && props.dataValues) {
    props.dataValues = _expandData(props.dataValues);
  ...
  Object.keys(props).forEach(key => {
    ...
    if (value && typeof value === 'object') {
      if (typeof value.mimeType !== 'undefined'
        ...
        const name = path.basename(value.fileName);
        const content = fs.readFileSync(value.filePath).toString('base64');

        props[key] = `data:${value.mimeType};name=${name};base64,${content}`;
      } else {
        props[key] = _expandData(value);
      }
    }
  });
...
module.exports = (deps, model, options) => {
  ...
  function ok(result) {
    ...
    instance.options.result = _expandData(instance.options.result || result);
