        ...
        tasks.push((row, isUpdate) => {
          ...
          if (_isNew) {
            return through && _value
              ...
              : (through && models[through].create(util.merge(item, props))) || row[assoc.accessors.create](item);
          }
          ...
          if (through) {
            /* istanbul ignore else */
            if (isUpdate) {
              return _options.noupdate !== true && models[through].update(util.merge(item, props), { where: _whereOpts });
            }

            return models[through].create(util.merge(_whereOpts, item, props));
          }

          return row[assoc.accessors.create](util.merge(item, props));
        });
      });
    }
  });
