...
function _pushKeys(model, obj) {
  Object.keys(model.rawAttributes).forEach(key => {
    ...
    /* istanbul ignore else */
    if (typeof prop === 'object' && (prop.primaryKey || prop.references) && obj.attributes.indexOf(prop.fieldName) === -1) {
...
module.exports = (conn, options, modelName) => {
  ...
  Object.keys(model.rawAttributes)
    .filter(x => typeof model.rawAttributes[x] === 'object' && model.rawAttributes[x].primaryKey).sort()
    .forEach((pk, i) => {
      /* istanbul ignore else */
  ...
  function build(payload, isUpdate, _options) {
    ...
    return Promise.resolve()
      ...
      .then(row => {
        ...
        if (!pk) {
          throw new Error(`Unable to retrieve ${model.name}.primaryKeyAttribute`);
        }

        return _tasks.reduce((prev, cur) => prev.then(() => cur(pk, _opts, isUpdate)), Promise.resolve()).then(() => pk);
      })
      .then(ok)
      .catch(err);
  }
