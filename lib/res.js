  ...
  Object.keys(inputData).forEach(prop => {
    ...
    if (ref && assoc) {
      ...
      items.forEach(item => {
        ...
        const _pk = Object.keys(_ref.primaryKeys);
        const _isNew = _value && !_pk.every(x => _value[x]);
        ...
        if (_value && typeof _value === 'object') {
          _pk.forEach(x => {
            item[_util[_ref.options.underscored ? 'underscore' : 'camelize'](`${ref.model}_${x}`)] = _value[x];
          });
        }

        tasks.push((row, isUpdate) => {
          const props = {
            [assoc.foreignKey]: row[fields[0]],
          };

          /* istanbul ignore else */
          if (_isNew) {
            return through && _value
              ? row[assoc.accessors.create](_value, { through: item })
              : (through && models[through].create(Object.assign(item, props))) || row[assoc.accessors.create](item);
          }

          /* istanbul ignore else */
          if (isUpdate && row[assoc.accessors.update]) {
            return _options.noupdate !== true && row[assoc.accessors.update](item);
          }
          ...
          if (through) {
            /* istanbul ignore else */
            if (isUpdate) {
              return _options.noupdate !== true && models[through].update(Object.assign(item, props), { where: _whereOpts });
            }

            return models[through].create(Object.assign(_whereOpts, item, props));
          }

          return row[assoc.accessors.create](Object.assign(item, props));
        });
      });
    }
  });
...
module.exports = (conn, options, modelName) => {
  ...
  function build(payload, isUpdate, _options) {
    ...
    const allPKs = _props[model.name].references.primaryKeys.map(x => x.prop);

    allPKs.forEach(key => {
      if (_props[key]) {
        if (!_payload[_props[key].model]) {
          delete _payload[key];
          return;
        }

        _props[key].references.primaryKeys.forEach(x => {
          _payload[key] = _payload[_props[key].model][x.prop];

          delete _payload[_props[key].model];
        });
      }
    });
    ...
    return Promise.resolve()
      ...
      .then(row => {
        return Promise.resolve()
          ...
          .then(foundRow => _tasks
            .reduce((prev, cur) => prev.then(() => cur(foundRow || row, isUpdate)), Promise.resolve())
            .then(() => foundRow || row));
      })
      .then(ok)
      .catch(err);
