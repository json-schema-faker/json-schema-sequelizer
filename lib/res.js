      ...
      if (Array.isArray(inputData[prop])) {
        inputData[prop].forEach(item => {
          ...
          if (ref.through) {
            ...
            if (item[ref.model]) {
              const _pk = ref.references.primaryKeys[0].prop;
              ...
              tasks.push((row, isUpdate) => {
                ...
                return Promise.resolve()
                  .then(() => {
                    ...
                    if (keys[1]) {
                      opts.where[keys[1]] = sub[_pk];
                    }

                    return models[through].update(item, opts);
                  })
                  .then(() => {
                    const opts = {
                      where: util.merge({
                    ...
                    return models[ref.model].update(sub, opts);
                  });
              });
            } else {
              tasks.push((row, isUpdate) => {
                /* istanbul ignore else */
                if (!isUpdate) {
                  return models[through].create(util.merge({
                    [keys[0]]: row[pk],
                  }, item, where));
                }
...
module.exports = (deps, models, options, modelName) => {
  ...
  function ok(result) {
    const data = _expandData(options.plain !== true, result, options.attachments || []);
