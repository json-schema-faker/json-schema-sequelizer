...
module.exports = (deps, model, options) => {
  ...
  function append(id) {
    Object.keys(deps[id].$schema.dependencies).forEach(dep => {
      const ref = deps[id].$schema.dependencies[dep];
      ...
      _props[ref.params.as] = {
        ...
        virtual: deps[ref.target].$schema.virtual || false,
        refs: util.copy(deps[ref.target].$schema.references),
      };

      _props[ref.params.as].refs.foreignKey = ref.foreignKey || null;
