...
function _buildTasks(references, inputData, modelName, _options, models) {
  ...
  Object.keys(inputData).forEach(prop => {
    ...
    const _attr = models[modelName].rawAttributes[prop] || {};

    /* istanbul ignore else */
    if (!_schema) {
      throw new Error(`Missing schema for '${prop}' (given ${JSON.stringify(inputData[prop])})`);
    }

    if (_attr.references && typeof inputData[prop] === 'object') {
      ...
      if (ref && assoc) {
        ...
        items.filter(Boolean).forEach(item => {
            ...
            Object.keys(_attrs).forEach(field => {
              ...
              if (_attrs[field].references && typeof item[field] === 'object') {
                ...
                tasks.before.push(_opts => _ref.create(item[field], { logging: _opts.logging })
                  .then(result => {
                    item[field] = _fixPkNull(result, _ref)[_pk];
                  }));
              }
            });
          }
          ...
          tasks.after.push((fk, opts, isUpdate) => {
            ...
            const _keys = Object.keys(_ref.primaryKeys);
            const _isNew = _value && !_keys.every(x => _value[x]);
            ...
            const refs = Object.keys(_ref.associations).reduce((memo, cur) => {
              /* istanbul ignore else */
              if (_value && _value[cur]) memo.push(_ref.associations[cur]);
              return memo;
            }, []);
            ...
            if (_isNew) {
              /* istanbul ignore else */
              if (through) {
                return _ref.create(_value || {}, { logging, include: refs })
                  .then(x => _fixPkNull(x, _ref))
                  .then(x => models[through].create(Object.assign({ [assoc.otherKey]: x[_pk] }, item, props)), { logging })
                  .then(x => _fixPkNull(x, models[through]));
              }
            } else if (through) {
              /* istanbul ignore else */
              ...
              return models[through]
                .create(Object.assign(item, props, _whereOpts), { logging, include: refs })
                .then(x => _fixPkNull(x, models[through]));
            } else if (isUpdate) {
              return _options.noupdate !== true && _ref
                .update(item, { where: Object.assign(props, _whereOpts), logging, include: refs });
            }

            /* istanbul ignore else */
            if (Array.isArray(inputData[prop]) && inputData[prop].length > 1) {
              return Promise.all(inputData[prop]
                .map(x => _ref.create(Object.assign(x, props), { logging, incude: refs }).then(y => _fixPkNull(y, _ref))));
            }

            return _ref.create(Object.assign(
              (Array.isArray(inputData[prop]) && inputData[prop][0]) || inputData[prop],
              typeof item === 'object' ? item : null,
              props), { logging, include: refs })
              .then(x => _fixPkNull(x, _ref));
          });
        });
    ...
    return Promise.resolve()
      ...
      .then(row => {
        ...
        if (!Array.isArray(row)) {
          pk = _fixPkNull(row, model)[model.primaryKeyAttribute];
        }
