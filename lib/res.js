...
module.exports = (deps, models, options, modelName) => {
  ...
  function ok(result) {
    ...
    Object.keys(_props).forEach(ref => {
      /* istanbul ignore else */
      if (Array.isArray(data[ref]) && _props[ref] && _props[ref].through) {
        data[ref] = data[ref].map(item => {
          const sub = item[_props[ref].through];

          delete item[_props[ref].through];

          sub[_props[ref].model] = item;
