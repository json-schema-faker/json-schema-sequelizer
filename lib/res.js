...
function _saveBase64(Model, key, payload, attachments, onUploadCallback) {
  ...
  if (typeof payload[key] === 'string' && payload[key].substr(0, 5) === 'data:') {
    ...
    const fileName = details[1] ? `_${details[1].split('name=')[1]}` : `.${details[0].match(/\/(\w+)\b/)[1]}`;
...
function _saveUploads(Model, field, payload, attachments, onUploadCallback) {
  ...
  Object.keys(Model.rawAttributes).forEach(prop => {
    ...
    if (payload[prop]
      ...
      const metadata = {
        mtime: attachment.lastModifiedDate,
      ...
      items.forEach(item => {
        ...
        if (_options.attachments) {
          if (item.$upload) {
            ...
            tasks.before.push(() => (typeof item === 'string' && RE_DATA.test(item)
              ? _saveBase64(_ref, prop, inputData, _options.attachments, _options.upload)
