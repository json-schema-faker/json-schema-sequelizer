...
function _fixParams(schema, props, data) {
  ...
  Object.keys(schema.properties).forEach(prop => {
    /* istanbul ignore else */
    if (props[prop] && props[prop].rel === 'BelongsTo') {
...
module.exports = (model, options) => {
  ...
  instance.actions = {
              ...
              Object.keys(_props).forEach(key => {
                ...
                data.forEach(child => {
                  ...
                  child[_props[key].foreignKey] = (whereOptions && whereOptions[_primaryKey])
                    || (options.where && options.where[_primaryKey])
                    || _payload[_primaryKey];
    ...
    create: payload =>
      model.create(_fixParams(model.options.$schema, _props, payload || options.payload), _getOpts(model, 'create'))
        .then(ok),
