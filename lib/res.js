...
module.exports = (model, options) => {
  ...
  Object.keys(_schema.properties).forEach(prop => {
    /* istanbul ignore else */
    if (_schema.properties[prop].$ref && _schema.properties[prop].virtual === true) {
      const config = util.getRefs(_schema.properties[prop]);

      // mock associations
      _props[prop] = {
        rel: config.method[0].toUpperCase() + config.method.substr(1),
        model: _schema.properties[prop].$ref,
        plural: _util.pluralize(_schema.properties[prop].$ref),
        singular: _util.singularize(_schema.properties[prop].$ref),
      };
    }
  });
