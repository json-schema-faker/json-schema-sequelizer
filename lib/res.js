    ...
    if (ref) {
      ...
      if (Array.isArray(inputData[prop])) {
        inputData[prop].forEach(item => {
          ...
          const isNew = _value && !_pk.every(x => _value[x]);

          delete item[ref.model];

          tasks.push(row => {
            const pks = ref.references.primaryKeys.map(x => x.prop);
            const fks = ref.references.foreignKeys.map(x => x.prop);

            const props = _getData(row, fks, pks);
            ...
            if (ref.through) {
              ...
              if (_value) {
                /* istanbul ignore else */
                if (!pks.every(x => item[x])) {
                  /* istanbul ignore else */
                  if (isNew) {
                    return row[_method](_value, { through: item });
                  }

                  return models[through].create(util.merge(item, props));
                }

                return models[through].update(util.merge(item, props), {
                  where: _getData(item, pks, pks),
                });
              }
            ...
            return isNew
              ? _ref.create(util.merge(item, props))
              : _ref.update(util.merge(item, props), { where: _getData(item, pks, pks) });
          });
        });
