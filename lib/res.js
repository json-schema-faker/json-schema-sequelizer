...
module.exports = (deps, models, options, modelName) => {
  ...
  function build(payload, isUpdate, whereOptions) {
    ...
    return model[isUpdate ? 'update' : 'create'](_payload, test.options)
      .then(row => {
        test.tasks.push(() => {
          return Promise.all(test.buildItems.map(cb => cb(row, isUpdate)));
        });

        return Promise.all(test.tasks.map(cb => cb(row, isUpdate))).then(() => row);
      })
      .then(ok)
      .catch(err);
  }
  ...
  instance.actions = {
    ...
    findAndCountAll: whereOptions => fetch('findAndCountAll', whereOptions),
