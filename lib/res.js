        ...
        inputData[prop].forEach(item => {
          ...
          if (ref.through) {
            ...
            if (item[ref.model]) {
              ...
              tasks.push(row => {
                const pk = models[through].primaryKeyAttribute;

                if (pks.length && !row[prop].map(x => x[through][pk]).includes(pks[0])) {
                  // FIXME: force is required to effectively remove paranoid rows!
                  return models[through].destroy({
                    force: true,
                    where: {
                      [pk]: pks[0],
                    },
                ...
                if (allFKs.every(x => where[x])) {
                  util.merge(where, _getData(row, allFKs, allPKs));
                ...
                /* istanbul ignore else */
                if (hasKeys) {
                  util.merge(rel, item, _getData(row, allFKs, allPKs));

                  // FIXME: find a better way to include/exclude from PKs/FKs?
                  Object.keys(models[modelName].primaryKeys).forEach(x => {
                    rel[modelName + x[0].toUpperCase() + x.substr(1)] = inputData[x];
                  });
