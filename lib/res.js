...
module.exports = (deps, model, options) => {
  ...
  function append(dependency, associations) {
    Object.keys(deps[dependency].$schema.dependencies).forEach(dep => {
      const ref = deps[dependency].$schema.dependencies[dep];
      const assoc = associations && associations[ref.params.as];
      const props = assoc ? deps[dependency].$schema.properties : {};
      ...
      _props[ref.params.as] = {
        ...
        virtual: deps[ref.target].$schema.virtual || null,
        fkey: assoc
          ? { prop: assoc.foreignKey, type: props[assoc.target.primaryKeyAttribute].type }
          : null,
        pkey: ref.primaryKey || null,
      };

      // append references from virtual-models
      Object.keys(props).forEach(prop => {
        const _ref = (props[prop].items || props[prop]).$ref;
  ...
  append(model.name, model.associations);
