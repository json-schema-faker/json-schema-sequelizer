            ...
            if (item[ref.model]) {
              ...
              tasks.push(row => {
                ...
                if (allFKs.every(x => where[x])) {
                  const found = row[prop].findIndex(x => {
                    for (let i = 0; i < allFKs.length; i += 1) {
                      if (x[through][allFKs[i]] !== where[allFKs[i]]) {
                        return false;
                      }
                    }
                  ...
                  if (found !== -1) {
                    Object.keys(row[prop][found][through].dataValues).forEach(v => {
                      if (v in item) {
                        const a = typeof item[v];
                        const b = typeof row[prop][found][through].dataValues[v];

                        if (a === b) {
                          row[prop][found][through].dataValues[v] = item[v];
                        }
                      }
                    });
                  }
