            ...
            if (item[ref.model]) {
              ...
              tasks.push(row => {
                ...
                if (hasKeys) {
                  ...
                  references[modelName].references.primaryKeys.forEach(x => {
                    if (typeof row[x.prop] !== 'undefined') {
                      // FIXME: how to retrieve real field if any?
                      rel[modelName + x.prop[0].toUpperCase() + x.prop.substr(1)] = row[x.prop];
                    }
                  });
              ...
              tasks.push(row => {
                ...
                if (keys.length && allFKs.every(x => mixedProps[x])) {
                  return models[through].create(util.merge(item, mixedProps));
                }
              });
            }
          }
