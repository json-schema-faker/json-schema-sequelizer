...
function makeRefs(models) {
  ...
  Object.keys(models).forEach(model => {
    ...
    Object.keys(a.refs).forEach(prop => {
      ...
      /* istanbul ignore else */
      if (b.method === 'hasMany') {
        refs[b.target] = refs[b.target] || {};
        refs[b.target][a.name] = models[b.target].belongsTo(a, {
          through: `${a.name}_${b.params.as}`,
          as: a.name,
        });
      }

      refs[a.name] = refs[a.name] || {};
      refs[a.name][b.params.as] = a[b.method](models[b.target], b.params);
    });
  });
