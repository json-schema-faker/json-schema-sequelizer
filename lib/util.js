...
function makeModel(ref, conn, model, schema) {
  ...
  const fixedModel = merge(copy(model), schema.options);
  const _model = conn.define(ref, _types.props, fixedModel);
...
function makeRefs(models, defns) {
  ...
  .forEach(a => {
    Object.keys(a.attributes).forEach(prop => {
      if (a.attributes[prop].references) {
        const refs = a.attributes[prop].references;
        const type = a.attributes[prop].type;

        // append associated refs
        a.options.$schema.properties[prop] =
        defns[a.name].$schema.properties[prop] = {
