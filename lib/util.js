...
function mixRefs(schema) {
  ...
  const fixedId = (['object', 'array'].indexOf(items.type) !== -1 && items._id)
    || items.type
    || items.$ref;
  ...
  Object.keys(models).map(model => {
    ...
    Object.keys(a.refs).forEach(prop => {
      ...
      if (models[b.target].model.virtual !== true) {
        ...
        if (b.params.through) {
          ...
          conn.models[through.model].options._hasForeignKeys = 1;
        }
    ...
    .forEach(a => {
      ...
      Object.keys(a.model.associations).forEach(prop => {
        ...
        a.model.options._hasForeignKeys = 2;
      });
    });
}
...
function sortModels(deps) {
  ...
  function max(values) {
    return Math.max.apply(null, values.map(x => sortedKeys.indexOf(x)));
  }

  sortedKeys.sort((a, b) => {
    /* istanbul ignore else */
    if (!tree[b].length || (map[a].options._hasForeignKeys < map[b].options._hasForeignKeys)) {
      return 1;
    }
    ...
    const left = max(tree[a]);
    const right = max(tree[b]);

    return left - right;
  });

  return sortedKeys.map(x => map[x]);
}
