...
function makeFK(properties, model, pk) {
  ...
  if (!properties[modelProp]) {
    ...
    if (pk === false) {
      ...
      delete properties[modelProp].primaryKey;
    }
  }
}
      ...
      if (models[b.target].model.virtual !== true) {
        ...
        if (b.params.through) {
          if (!(models[b.params.through] || defns[b.params.through])
            ...
            // sync foreign-keys
            makeFK(defns[b.params.through].$schema.properties, a.model);
            makeFK(defns[b.params.through].$schema.properties, models[b.target].model);
        ...
        if (b.params.foreignKey) {
          makeFK(defns[b.target].$schema.properties, a.model, false);
          makeFK(conn.models[b.target].options.$schema.properties, a.model, false);
