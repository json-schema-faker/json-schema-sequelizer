...
function makeModel(ref, conn, defns, schemas) {
  ...
  Object.keys(schemas).forEach(k => {
    _schema.definitions[k] = schemaTypes.cleanSchema(schemas[k]);
  });

  _model.faked = fakeSchema(fixRefs(_schema));
...
function makeRefs(models, defns) {
  Object.keys(models).map(model => {
    ...
    Object.keys(a.refs).forEach(prop => {
      ...
      a.model[b.method](models[b.target].model, b.params);
    });
