...
function fixRefs(schema, resolve) {
  /* istanbul ignore else */
  if (resolve) {
    reduceRefs(schema);
  }
  ...
  if (schema && typeof schema === 'object') {
    ...
    if (Array.isArray(schema)) {
      return schema.map(x => fixRefs(x, resolve));
    }
    ...
    Object.keys(schema).forEach(key => {
      if (resolve && typeof schema[key].id === 'string') {
        schema[key] = { $ref: schema[key].id };
      } else {
        schema[key] = fixRefs(schema[key], resolve);
      }
    });

    /* istanbul ignore else */
    if (!resolve && typeof schema.id === 'string') {
      delete schema.id;
    }
  }
