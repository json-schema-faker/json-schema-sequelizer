...
function makeModel(ref, conn, defns, schemas) {
  ...
  if (schema.virtual === true) {
    ...
    const fixedModel = merge(model, schema.options);

    _model = conn.define(ref, _types.props, fixedModel);
...
function makeRefs(models, defns) {
  Object.keys(models).map(model => {
    ...
    Object.keys(a.refs).forEach(key => {
      const deps = defns[a.refs[key].target].$schema.dependencies;

      Object.keys(deps).forEach(dep => {
        const _model = models[deps[dep].target].model;
        const _pk = _model.primaryKeyAttribute;

        deps[dep].primaryKey = {
          prop: _pk,
          type: _model.options.$schema.properties[_pk].type,
        };
      });
    });
